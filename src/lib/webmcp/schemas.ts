...
export const SearchProjectsSchema = z.object({
  query: z.string().trim().max(100).optional().describe("Free-form text query for search"),
  domain: z
    .string().trim().max(50)
    .optional()
...
export const FindRelevantWorkSchema = z.object({
  role: z.string().trim().max(100).optional().describe("Target role (e.g., 'Backend Engineer')"),
  skills: z.array(z.string().trim().min(1).max(50)).max(10).optional().describe("Relevant skills (e.g., ['Go', 'Docker'])"),
  domains: z.array(z.string().trim().min(1).max(50)).max(10).optional().describe("Relevant domains (e.g., ['Distributed Systems'])"),
  keywords: z.array(z.string().trim().min(1).max(50)).max(10).optional().describe("General keywords"),
...
export const DraftContactSchema = z.object({
  ...
  lastName: z.string().trim().max(100).optional().describe("User's last name"),
  email: z.string().trim().max(150).email().describe("User's email address"),
  message: z.string().trim().max(2000).describe("The message or project inquiry from the user"),
}).strict();

export const GetProjectSchema = z.object({
  slug: z.string().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/).describe("The slug of the project (e.g. lexum, raptor)"),
}).strict();

export const GetArticleSchema = z.object({
  slug: z.string().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/).describe("The slug of the article (e.g. tech-stack-wars)"),
}).strict();
